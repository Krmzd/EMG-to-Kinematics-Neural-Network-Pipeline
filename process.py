...
class Preprocess:
    ...
    def __init__(self, cut_off = 6.0, fs_emg = 1000, fs_kin = 100):
        ...
        if fs_emg % fs_kin !=0:
            raise ValueError(f"EMG ({fs_emg}) must be a multiple of Kinematics ({fs_kin})")
    ...
    def process_emg(self, emg_df: pd.DataFrame) -> pd.DataFrame:
        ...
        emg_filtered = emg_rectified.apply(self._butter_lowpass_filter) # Low-pass Filter (Linear Envelope)
        
        return (
            emg_filtered.groupby(np.arange(len(emg_filtered)) // self.ratio).mean().reset_index(drop=True) 
        ) # Downsample (Take mean of every 10 rows)

    
    def extract_synced_trial(self, file_path: Path, feature_map: Dict, target_map: Dict) -> pd.DataFrame:
        # Locate the anchors
        ...
        # Extract EMG
        emg_header = pd.read_csv(file_path, skiprows= emg_idx + 2, nrows= 1, header= None).iloc[0].tolist()
        ...
        emg_raw = pd.read_csv(file_path, usecols= emg_col_indices, skiprows= emg_idx + 4, nrows= num_emg_rows)
        emg_clean = self.process_emg(emg_raw).rename(columns= feature_map) # Process: NaNs, -> Rectifu -> Filter -> Downsample
        
        # Read the header row for Model Outputs
        kin_header = pd.read_csv(file_path, skiprows=model_idx + 2, nrows=1, header=None).iloc[0].tolist()
        ...
        kin_raw = pd.read_csv(file_path, usecols = target_map, skiprows = model_idx + 4) # Extract the target 
        kin_clean = self._handle_nans(kin_raw).reset_index(drop=True).rename(columns=target_map) # clean kinematics
        
        # Final sync
        min_len = min(len(emg_clean), len(kin_clean)) # Sync lengths and stitch
        final_trial = pd.concat([emg_clean.iloc[:min_len], kin_clean.iloc[:min_len]], axis = 1)
        return final_trial

    def process_subject(self, folder_path: str, side_config: Dict) -> Tuple[pd.DataFrame, Dict]:
        """
        Main coordinator:
        ...
        for f in files:
            try:
                right_trials.append(self.extract_synced_trial(f, side_config["Right"]["features"], side_config["Right"]["target"]))
                left_trials.append(self.extract_synced_trial(f, side_config["Left"]["features"], side_config["Left"]["target"]))
